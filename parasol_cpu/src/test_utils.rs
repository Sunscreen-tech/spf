...
use parasol_runtime::{
    test_utils::{
        get_secret_keys_128, get_secret_keys_80, get_compute_keys_128, get_compute_keys_80,
    },
...
pub fn make_computer_80() -> (FheComputer, Encryption) {
    let compute_keys = get_compute_keys_80();
    let enc = Encryption::new(&DEFAULT_80);
    let eval = Evaluation::new(compute_keys, &DEFAULT_80, &enc);
...
pub fn make_computer_128() -> (FheComputer, Encryption) {
    ...
    let eval = Evaluation::new(compute_keys, &DEFAULT_128, &enc);
