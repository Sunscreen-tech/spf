...
#[ignore]
#[test]
fn can_run_auction_optimized_from_elf() {
    let memory = Arc::new(Memory::new_from_elf(include_bytes!("../test_data/auction")).unwrap());
    ...
    let scratch = std::array::from_fn::<_, 16, _>(|_| UInt::<32, _>::new(&enc));
    let scratch = memory.try_allocate_type(&scratch).unwrap();
    ...
    let prog = memory.get_function_entry("auction_optimized").unwrap();
