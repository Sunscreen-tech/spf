...
public:
    ...
    __device__ inline GlevCiphertext<T>* rows(uint32_t i, const GlevSizeInfo &size_info) {
        auto as_array = reinterpret_cast<DstArray<GlevCiphertext<T>> *>(this, size_info.get<0>());
        return as_array->nth(i);
    }

    __device__ inline GlevCiphertext<T>* rows(uint32_t i, const GlevSizeInfo &size_info) const {
        auto as_array = reinterpret_cast<DstArray<GlevCiphertext<T>> *>(this, size_info.get<0>());
        return as_array->nth(i);
    }
private:
