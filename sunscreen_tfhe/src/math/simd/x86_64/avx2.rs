...
#[inline]
#[target_feature(enable = "avx2,fma")]
pub fn complex_add_avx2(c: &mut [Complex<f64>], a: &[Complex<f64>], b: &[Complex<f64>]) {
    for ((c, a), b) in c.iter_mut().zip(a.iter()).zip(b.iter()) {
        *c = a + b;
    }
}
...
#[inline]
#[target_feature(enable = "avx2,fma")]
pub fn complex_sub_avx2(c: &mut [Complex<f64>], a: &[Complex<f64>], b: &[Complex<f64>]) {
    for ((c, a), b) in c.iter_mut().zip(a.iter()).zip(b.iter()) {
        *c = a - b;
    }
}
...
#[inline]
#[target_feature(enable = "avx2,fma")]
pub fn complex_sub_assign_avx2(c: &mut [Complex<f64>], b: &[Complex<f64>]) {
...
#[target_feature(enable = "avx2,fma")]
#[inline]
pub fn vector_shr_round(c: &mut [u64], a: &[u64], n: u32) {
    for (c, a) in c.iter_mut().zip(a.iter()) {
        let round_bit = (*a >> (n - 1)) & 0x1;
