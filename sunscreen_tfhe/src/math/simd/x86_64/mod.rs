...
pub fn complex_msub(c: &mut [Complex<f64>], a: &[Complex<f64>], b: &[Complex<f64>]) {
    ...
    if avx_512_available() {
        unsafe { avx512::complex_msub_avx_512_unchecked(c, a, b) }
    } else if fma_available() && avx2_available() {
        unsafe { avx2::complex_msub_avx2(c, a, b) }
    } else {
        scalar::complex_mad(c, a, b)
    }
}
...
pub fn complex_mul(c: &mut [Complex<f64>], a: &[Complex<f64>], b: &[Complex<f64>]) {
    ...
    if fma_available() && avx2_available() {
        unsafe { avx2::complex_mul_avx2(c, a, b) }
    } else {
...
/// Compute vector `c -= b` over &[Complex<f64>].
///
/// # Panics
/// If `c.len() != b.len()`
/// If `b.len() % 8 != 0`
/// If `b`, `c` are not aligned to a 512-bit boundary.
