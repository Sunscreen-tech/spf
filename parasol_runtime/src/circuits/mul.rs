...
pub fn append_int_multiply<OutCt: Muxable>(
    ...
    let abs_a = abs::<OutCt>(uop_graph, a);
    let abs_b = abs::<OutCt>(uop_graph, b);

    let pos_product = mul_impl::<OutCt>(uop_graph, &abs_a, &abs_b);
    let neg_product = neg::<OutCt>(uop_graph, &pos_product);

    let should_neg = xor::<OutCt>(uop_graph, *a.last().unwrap(), *b.last().unwrap());

    let result = mux::<OutCt>(uop_graph, &pos_product, &neg_product, should_neg);
...
fn neg<OutCt: Muxable>(uop_graph: &mut FheCircuit, input: &[NodeIndex]) -> Vec<NodeIndex> {
    // flip every bit
    let flipped = input
        .iter()
        .map(|&input_node| {
            let not_node = uop_graph.add_node(FheOp::Not);
            let output_node = uop_graph.add_node(OutCt::MUX_MODE.one());
            uop_graph.add_edge(input_node, not_node, FheEdge::Unary);
            uop_graph.add_edge(not_node, output_node, FheEdge::Unary);
            output_node
        })
        .collect::<Vec<_>>();
    ...
    let one = uop_graph.add_node(OutCt::MUX_MODE.one());
    uop_graph
        .insert_mux_circuit(
            &add_circuit,
...
fn abs<OutCt: Muxable>(uop_graph: &mut FheCircuit, input: &[NodeIndex]) -> Vec<NodeIndex> {
    // use original MSB to select between original and negated
    let neg_input = neg::<OutCt>(uop_graph, input);

    mux::<OutCt>(uop_graph, input, &neg_input, *input.last().unwrap())
}

/// Helper function for signed integer multiplication: select number by one bit
fn mux<OutCt: Muxable>(
    uop_graph: &mut FheCircuit,
    val_for_false: &[NodeIndex],
    val_for_true: &[NodeIndex],
    sel: NodeIndex,
) -> Vec<NodeIndex> {
    val_for_false
        .iter()
        .zip(val_for_true.iter())
        .map(|(&bit_for_false, &bit_for_true)| {
            ...
            uop_graph.add_edge(cmux_node, output_node, FheEdge::Unary);
            output_node
        })
        .collect()
}

/// Helper function for signed integer multiplication: xor two bit
fn xor<OutCt: Muxable>(
    uop_graph: &mut FheCircuit,
    a_node: NodeIndex,
    b_node: NodeIndex,
) -> NodeIndex {
    // create not b
    let not_node = uop_graph.add_node(FheOp::Not);
    let nb_node = uop_graph.add_node(OutCt::MUX_MODE.one());
    uop_graph.add_edge(b_node, not_node, FheEdge::Unary);
    uop_graph.add_edge(not_node, b_node, FheEdge::Unary);

    // then use a as sel to select between b and not b
    let cmux_node = uop_graph.add_node(FheOp::CMux);
    let output_node = uop_graph.add_node(OutCt::MUX_MODE.one());
    uop_graph.add_edge(a_node, cmux_node, FheEdge::Sel);
    uop_graph.add_edge(b_node, cmux_node, FheEdge::Low);
    uop_graph.add_edge(nb_node, cmux_node, FheEdge::High);
    uop_graph.add_edge(cmux_node, output_node, FheEdge::Unary);
