...
use crate::{
    circuits::mul::append_int_multiply, crypto::PublicKey, safe_bincode::GetSize, Encryption,
...
/// A collection of graph nodes resulting from FHE operations over signed integers (e.g. the
/// result of adding two 7-bit signed values).
///
/// # Remarks
/// This integer is in unpacked form, meaning each bit resides in a different ciphertext of type `T`.
pub struct IntGraphNodes<'a, const N: usize, T: CiphertextOps> {
    /// The signed integer's [`BitNode`]s from least to most significant.
...
impl<'a, const N: usize, T: CiphertextOps> IntGraphNodes<'a, N, T> {
    ...
    /// Convert this [`IntGraphNodes<T>`] to a [`IntGraphNodes<U>`]. Usually, you'll use this
    /// to convert to [`L1GgswCiphertext`] so you can perform arithmetic computation over integers.
    pub fn convert<U: CiphertextOps>(&self, graph: &'a FheCircuitCtx) -> IntGraphNodes<'a, N, U> {
        let iter = self.bits.iter().map(|x| x.convert(graph));

        IntGraphNodes::from_bit_nodes(iter, &graph.allocator)
    }

    /// Add output nodes to the computation for each of this signed integer's bits.
    /// When the [`FheCircuitCtx`]'s DAG finishes computing, the returned [`Int<N, T>`] will encrypt
    /// the result of this signed integer's DAG nodes.
    ///
    /// # Remarks
    /// The returned [`Int`] has not yet been evaluated and will be a trivial zero until the
    ...
    /// is running.
    pub fn collect_outputs(&self, ctx: &FheCircuitCtx, enc: &Encryption) -> Int<N, T> {
        let result = Int::new(enc);
...
impl<const N: usize> IntGraphNodes<'_, N, L1GlweCiphertext> {
    /// Convert this unpacked signed integer to packed form.
    ///
    /// # Remarks
    ...
    pub fn pack(
        ...
        ctx: &FheCircuitCtx,
        enc: &Encryption,
    ) -> PackedIntGraphNode<N, L1GlweCiphertext> {
...
impl<'a, const N: usize> IntGraphNodes<'a, N, L1GgswCiphertext> {
    pub(crate) fn cmp<const M: usize, OutCt: Muxable>(
        &self,
        other: &IntGraphNodes<M, L1GgswCiphertext>,
        ctx: &FheCircuitCtx,
        gt: bool,
        eq: bool,
    ) -> BitNode<OutCt> {
...
/// A graph node that represents an signed integer in packed form. See [`PackedInt`] for a
/// description of packing.
pub struct PackedIntGraphNode<const N: usize, T: CiphertextOps + PolynomialCiphertextOps> {
    id: NodeIndex,
    _phantom: PhantomData<T>,
}

impl<const N: usize> PackedIntGraphNode<N, L1GlweCiphertext> {
    /// Convert this integer into unpacked form, where each bit appears in a different ciphertext.
    pub fn unpack<'a>(&self, ctx: &'a FheCircuitCtx) -> IntGraphNodes<'a, N, L1LweCiphertext> {
        let nodes = (0..N).map(|i| {
            let mut circuit = ctx.circuit.borrow_mut();
        ...
        IntGraphNodes::from_nodes(nodes, &ctx.allocator)
    }
}

impl<const N: usize, T: CiphertextOps + PolynomialCiphertextOps> PackedIntGraphNode<N, T> {
    ...
    /// a panic if you attempt to read the ciphertext while [`crate::UOpProcessor::spawn_graph`]
    /// is running.
    pub fn collect_output(&self, ctx: &FheCircuitCtx, enc: &Encryption) -> PackedInt<N, T> {
...
/// An signed integer store in unpacked form. An `N`-bit signed integer encrypts its bits in
/// `N` different ciphertexts of type `T`.
pub struct Int<const N: usize, T: CiphertextOps> {
    /// The ciphertexts encrypting this signed integer's bits in least-to-most significant order.
    pub bits: Vec<Arc<AtomicRefCell<T>>>,
}

impl<const N: usize, T: CiphertextOps> GetSize for Int<N, T> {
...
impl<const N: usize, T> Int<N, T>
where
    T: CiphertextOps,
{
    /// Allocate a new Int using trivial or precomputed (if T is [`L1GgswCiphertext`]) encryptions
    /// of zero.
    ...
    /// Create a [`Int`] from a previously encrypted set of type `T` ciphertexts.
    ///
    /// # Remarks
    ...
    /// Decrypts this encrypted integer and returns the contained Int message.
    ...
    pub fn graph_inputs<'a>(&self, ctx: &'a FheCircuitCtx) -> IntGraphNodes<'a, N, T> {
        IntGraphNodes::from_nodes(
            self.bits
                .iter()
...
///
/// Packed integers must be unpacked (with [`PackedIntGraphNode::unpack`]) before you can perform
/// computation.
///
/// # Example
/// ```rust
...
/// #   L0LweCiphertext, L1GlweCiphertext, DEFAULT_128, fluent::{FheCircuitCtx, PackedInt}
/// # };
...
/// ```
pub struct PackedInt<const N: usize, T>
where
...
impl<const N: usize, T> From<T> for PackedInt<N, T>
where
    T: CiphertextOps + PolynomialCiphertextOps,
{
...
impl<const N: usize, T: CiphertextOps + PolynomialCiphertextOps> GetSize for PackedInt<N, T> {
    fn get_size(params: &crate::Params) -> usize {
        T::get_size(params)
    }
...
impl<const N: usize, T> PackedInt<N, T>
where
    T: CiphertextOps + PolynomialCiphertextOps,
{
    /// Encrypt and pack the given `val` into a single `T` ciphertext.
    /// See [`PackedInt`] for more details on packing.
    ...
    /// Decrypt this packed encrypted signed integer.
    pub fn decrypt(&self, enc: &Encryption, sk: &SecretKey) -> u64 {
        assert!(N < T::poly_degree(&enc.params).0);
        let mut val = 0;
    ...
    pub fn graph_input(&self, ctx: &FheCircuitCtx) -> PackedIntGraphNode<N, T> {
        PackedIntGraphNode {
    ...
    /// Trivially encrypt the given value as a [`PackedInt`].
...
mod tests {
    ...
    #[test]
    fn can_roundtrip_packed_nt() {
        let enc = get_encryption_128();

        let sk = get_secret_keys_128();
        let pk = get_public_key_128();

        let val = PackedInt::<16, L1GlweCiphertext>::encrypt(65494, &enc, &pk); // 2^16 - 42 = 65494

        assert_eq!(val.decrypt(&enc, &sk), 65494);
    }
    ...
    fn can_unpack_int() {
        ...
        assert_eq!(as_unpacked.decrypt(&enc, &sk), 65494);
    }
    ...
    fn can_pack_int() {
        ...
        let val = Int::<15, L1GlweCiphertext>::encrypt_secret(32726, &enc, &sk); // 2^15 - 42 = 32726
        ...
        assert_eq!(actual.decrypt(&enc, &sk), 32726);
    }
    ...
    fn can_safe_deserialize_int() {
        fn case<T: CiphertextOps + for<'a> Deserialize<'a> + Serialize>() {
            ...
            let val = Int::<15, T>::encrypt_secret(32726, &enc, &sk); // 2^15 - 42 = 32726
    ...
    fn can_safe_deserialize_packed_int() {
        ...
        let val = PackedInt::<15, L1GlweCiphertext>::encrypt(32726, &enc, &pk); // 2^15 - 42 = 32726
    ...
    fn can_trivial_encrypt_packed_int() {
        ...
        let val = PackedInt::<15, L1GlweCiphertext>::trivial_encrypt(32726, &enc); // 2^15 - 42 = 32726

        assert_eq!(val.decrypt(&enc, &sk), 32726);
    }
}
